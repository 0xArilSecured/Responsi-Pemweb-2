<x-layout titleHeader="Login">    
    <main class="bg-white overflow-clip shadow-xl rounded-4xl min-w-7/12 h-[500px] flex" data-aos="fade-up">
        <div class="relative w-1/2 flex flex-col h-full justify-center py-5 px-20">    
            <p data-aos="fade-right" data-aos-delay="300" class="absolute top-5 right-5 w-full flex justify-end gap-1 text-xs text-gray-400">Don't have an account? <a class="text-primary" href="{{ route('register') }}">Sign Up</a></p>
            <div data-aos="fade-up" data-aos-delay="400">
                <h2 class="font-bold text-4xl mb-2">Login</h2>
                <p class="text-xs text-gray-400 mb-5">Organize Your Employees with EmpSecured</p>
            </div>
            <form data-aos="fade-up" data-aos-delay="500" method="POST" action="{{ route('login') }}">
                @csrf
                ...
                <button data-aos="zoom-in" data-aos-delay="600" class="text-xs font-medium cursor-pointer mt-5 py-3 rounded-full w-full bg-primary text-white hover:bg-primary-hover hover:shadow hover:scale-105 transition" type="submit">Login</button>
            </form>
        </div>
    ...
    @if(session('success'))
        <div 
            ...
            class="fixed bottom-8 left-1/2 -translate-x-1/2 z-50 bg-green-100 text-green-700 px-4 py-2 rounded shadow text-xs"
